...
bool compare_contours_len(const std::vector<cv::Point> &c1,
...
public:
    ...
    double FPS;
    float small_energy =
        20 * 3.14159265358979323846 / 60;  // 小能量机关角速度(rad/s)

    Buff(uint init_color) { color = init_color; }
    ...
    cv::Point2f GetArmorCenter() { return armor_center; }

    std::vector<cv::Point2f> GetImagePoints() { return imagePoints; }

    bool IfFindBuff()
    {
        return this->armor_center.x == -1 && this->armor_center.y == -1 ? false
                                                                        : true;
    }
    ...
    void ImageProcess(cv::Mat src)
    {
        ...
        cv::findContours(mat, contours, hierarchy, cv::RETR_EXTERNAL,
        ...
        cv::findContours(black_img, contours_, hierarchy_, cv::RETR_EXTERNAL,
                         cv::CHAIN_APPROX_SIMPLE);
        cv::Mat result_img = cv::Mat::zeros(src.size(), CV_8UC3);
        cv::drawContours(result_img, contours_, -1, cv::Scalar(255, 255, 255),
                         3, 4);
    ...
    void FindCenter()
    {
        ...
        std::vector<std::vector<cv::Point>> contours;
        cv::findContours(clone_img, contours, cv::RETR_EXTERNAL,
                         cv::CHAIN_APPROX_SIMPLE);
        if (!contours.empty())
        {
            std::sort(contours.begin(), contours.end(),
                      compare_contours_len);  // 根据轮廓长度对轮廓进行排序
            ...
            for (const auto &contour : contours)
            {
                ...
                cv::circle(clone_img, center, radius, cv::Scalar(255, 255, 255),
                           1);
            ...
            cv::polylines(clone_img, boxPointsCov, true,
                          cv::Scalar(255, 255, 255), 2);
            cv::circle(clone_img, rect_points[0], 5, cv::Scalar(255, 255, 255),
                       1, 8, 0);
        ...
        if (!armor_data.empty())
        {
            ...
            for (const auto &contour : contours)
            {
                all_points.insert(all_points.end(), contour.begin(),
                                  contour.end());
            }
            minEnclosingCircle(all_points, energy_organ_center,
                               radius_energy_organ);
            circle(clone_img, energy_organ_center, radius_energy_organ,
                   cv::Scalar(255, 255, 255), 2);  // 绘制最小外接圆
            circle(clone_img, energy_organ_center, 5, cv::Scalar(255, 255, 255),
                   1, 8, 0);
            radius_armor_center = cv::norm(
                energy_organ_center -
                armor_center);  // energy_organ_center到armor_center的距离为半径
            ...
            cv::Vec4f line(this->energy_organ_center.x,
                           this->energy_organ_center.y, this->armor_center.x,
                           this->armor_center.y);
            cv::Point point(this->energy_organ_center.x,
                            this->energy_organ_center.y);
            ...
            cv::circle(clone_img, armor_center, 5, cv::Scalar(255, 255, 255), 1,
                       8,
                       0);  // 画出装甲板中心
     ...
     * @return cv::Point2f
     */
    // cv::Point2f Predict(cv::Mat img, cv::Point2f energy_organ_center,
    // cv::Point2f armor_center, float radius_armor_center)
    // {
    //     // 先对识别到的能量机关用roi进行持续跟随并获取能量机关参数
    //     cv::RotatedRect roi_rect(this->energy_organ_center,
    //     cv::Size2f(this->radius_energy_organ * 2, this->radius_energy_organ *
    //     2), 0); cv::Rect roi = roi_rect.boundingRect(); cv::rectangle(img,
    //     roi, cv::Scalar(255, 255, 255), 1, 8, 0); cv::namedWindow("src");
    //     cv::imshow("src", img);

    //     //
    //     首先检测能量机关的线速度和角速度(在ROI区域中计算),并判断大小能量机关
    //     cv::Point2f point_roi = this->armor_center - cv::Point2f(roi.x,
    //     roi.y);
    //     // 转换为在roi区域的坐标

    //     armor_center_last.resize(200, cv::Point2f(0, 0)); //
    //     初始化2s的数据量，每个point先初始化为(0,0)
    //     std::vector<cv::Point2f>::iterator k =
    //     this->armor_center_last.begin(); armor_center_last.erase(k); //
    //     删除头元素 armor_center_last.push_back(point_roi); //
    //     在尾部插入新元素
    //     // 检查转换后的坐标是否在 ROI 区域内
    //     if (roi.contains(point_roi))
    //     {
    //         std::cout << "Point in ROI: (" << point_roi.x << ", " <<
    //         point_roi.y << ")"
    //                   << "\n";
    //     }
    ...
    //         this->judge_points.erase(this->judge_points.begin()); //
    //         删除头元素
    //     this->judge_points.push_back(temp);                       //
    //     在尾部插入新元素
    //     // end为失效的迭代器
    ...
    void AngleSolve()
    {
        if (this->angle_Energy_organ_center2armor_center >= 0 &&
            this->angle_Energy_organ_center2armor_center < 90)
        {
        ...
        else if (this->angle_Energy_organ_center2armor_center >= 90 &&
                 this->angle_Energy_organ_center2armor_center < 180)
        {
            this->imagePoints.push_back(this->imagePoints_[1]);
            this->imagePoints.push_back(this->imagePoints_[0]);
            this->imagePoints.push_back(this->imagePoints_[2]);
        ...
        else if (this->angle_Energy_organ_center2armor_center >= 180 &&
                 this->angle_Energy_organ_center2armor_center < 270)
        {
            this->imagePoints.push_back(this->imagePoints_[2]);
            this->imagePoints.push_back(this->imagePoints_[1]);
            this->imagePoints.push_back(this->imagePoints_[3]);
        ...
        else if (this->angle_Energy_organ_center2armor_center >= 270 &&
                 this->angle_Energy_organ_center2armor_center <= 360)
        {
            this->imagePoints.push_back(this->imagePoints_[3]);
            this->imagePoints.push_back(this->imagePoints_[2]);
            this->imagePoints.push_back(this->imagePoints_[0]);
