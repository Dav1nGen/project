...
public:
    explicit QrCodeDetector(std::string name) : Node(name)
    {
        ...
        // this->image_ = cv::imread("/home/made4/code/workspace/src/mypkg/qrcode_detector/img/1.jpg");
        // assert(!image_.empty() && "image empty!");
        // this->pnpSolveQrcode(this->image_);
        // this->image_center_point_ = cv::Point2f(this->image_.cols / 2, this->image_.rows / 2);

        //  创建发布者
        move_data_publisher_ptr_=this->create_publisher<mypkg_interfaces::msg::MoveData>(
                "move", 1);  //  发布者对象指针,用于在定时器中发布基于二维码位置的移动指令
...
private:
    ...
    rclcpp::Publisher<std_msgs::msg::Bool>::SharedPtr found_qrcode_publisher_ptr_;
    rclcpp::Publisher<mypkg_interfaces::msg::MoveData>::SharedPtr move_data_publisher_ptr_;
...
void QrCodeDetector::getParameter()
{
    ...
    this->camera_matrix_data_ = cv::Mat(3, 3, CV_64F,
                                    const_cast<double *>(this->declare_parameter("camera_matrix", std::vector<double>()).data()))
                                    .clone();
    this->dist_coeffs_data_ = cv::Mat(1, 5, CV_64F,
                                    const_cast<double *>(this->declare_parameter("dist_coeffs", std::vector<double>()).data()))
                                  .clone();
    //  设置棋盘格角点
    this->object_points_.push_back(cv::Point3f(0, qrcode_size / 2, -qrcode_size / 2));
    this->object_points_.push_back(cv::Point3f(0, qrcode_size / 2, qrcode_size / 2));
    this->object_points_.push_back(cv::Point3f(0, -qrcode_size / 2, qrcode_size / 2));
    this->object_points_.push_back(cv::Point3f(0, -qrcode_size / 2, -qrcode_size / 2));
...
void QrCodeDetector::pnpSolveQrcode(cv::Mat image)
{
    ...
    this->qrcode_center_point_ = cv::Point2f((this->image_points_[0].x+this->image_points_[2].x)/2, (this->image_points_[0].y+this->image_points_[2].y)/2);
    this->image_center_point_ = cv::Point2f(this->image_.cols / 2, this->image_.rows / 2);
    // cv::drawChessboardCorners(image, cv::Size(2, 2), this->image_points_, true);
    // cv::imshow("image", image);
    // cv::waitKey(0);
    //  EPnP解算
    bool solve = cv::solvePnP(this->object_points_, this->image_points_, this->camera_matrix_data_, this->dist_coeffs_data_, this->rvec_, this->tvec_, false, cv::SOLVEPNP_EPNP);
    //  打印解算结果
    if (solve)
    {
        RCLCPP_INFO(this->get_logger(), "Solve success");
        this->found_qrcode_.data=true;  //  解算成功才是真成功！！然后继续进行下一步
        // 计算相机与目标的距离
...
void QrCodeDetector::subscriptionCallback(const sensor_msgs::msg::Image::SharedPtr msg)
{
    //  将订阅者中的数据赋值给image_
...
void QrCodeDetector::pubMoveDataCallback(){
    ...
    //  二维码中心x坐标小于图像中心坐标，左转
    if(this->qrcode_center_point_.x<this->image_center_point_.x && std::abs(this->qrcode_center_point_.x-this->image_center_point_.x )>50)
    {
